...
public class CommHandler implements
    ...
    private class NumberSenderAsync extends AsyncTask<Integer, Void, Void> {
        ...
        protected Void doInBackground(Integer... params) {
            PutDataMapRequest putDataMapRequest = PutDataMapRequest.create("/" + CURRENT_WCHOICE_INDEX);
            putDataMapRequest.getDataMap().putInt(CURRENT_WCHOICE_INDEX, params[0]);
    ...
    public void onDataChanged(DataEventBuffer dataEventBuffer) {
        for(DataEvent event:dataEventBuffer){
            ...
            if(event.getType()== DataEvent.TYPE_CHANGED){
                ...
                if(item.getUri().getPath().compareTo("/"+ CURRENT_MCHOICE_INDEX)==0){
